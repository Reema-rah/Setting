...
app.post("/updateUser", async (req, res) => {
  ...
  console.log(email)
  try {
    await UserModel.updateOne({ _id:userID }, { $set: { email: email , username:name} })
    res.status(201).json();
...
app.post("/login", async (req, res) => {
  ...
  return res.json({ token, userID: user._id,avatar: user.avatar , username:user.username,email:user.email, message: "Success" })
})
